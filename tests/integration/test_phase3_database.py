...
try:
    ...
    print(f"   [OK] Schema created with tables: {', '.join(sorted(required_tables))}")
    ...
    print(f"       (Target: <1ms, Achieved: {'✓' if cached_time*1000 < 1 else '✗'})")
    
    # Check cache stats
    cache_info = retriever.get_hamnosys.cache_info()
    print(f"   [OK] Cache stats: {cache_info.hits} hits, {cache_info.misses} misses")
    ...
    # Verify seed data
    with db_manager.get_connection() as conn:
        count = conn.execute("SELECT COUNT(*) FROM gloss_mapping").fetchone()[0]
    ...
    print(f"   [OK] Multiple connections returned consistent data: {results[0]}")
